...
Request(EventCreate): cameraId(CameraId), detectedClass(DetectedClass), isMisclassified(bool), confidenceScore(float 0~1) — 전부 필수

Response(Event, 200): eventId(uuid), timestamp(ISO8601), cameraId, detectedClass, isMisclassified, confidenceScore, actionTaken(ActionTaken), imageFileId(str|null, GridFS, Mock단계 null), notes(str|null)
- isMisclassified=false 또는 5초 Cooldown 중이면 null 반환

부수효과: mode=MANAGE → RPA트리거+WS MISCLASSIFICATION_DETECTED 브로드캐스트 / mode=COLLECT → 통계만 갱신. 동일 cameraId+detectedClass 5초 내 재호출 무시(Cooldown)
...
| ID | Path | 템플릿 | 설명 |
|---|---|---|---|
| PG-01 | GET / | index.html | 카메라 3대 스트리밍+사이드바(모드토글,최근이벤트)+통계. mode를 컨텍스트로 전달(새로고침 시 상태유지) |
| PG-02 | GET /events | eventsList.html | EP-03 결과 표 렌더링 |
| PG-03 | GET /events/{id} | eventDetail.html | EP-04 결과 렌더링 |
| PG-04 | GET /statistics | statistics.html | EP-05 결과 Chart.js 렌더링 |
