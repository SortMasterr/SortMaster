...
| EP-16 | GET /api/binPositionMonitor/status, POST /api/binPositionMonitor/calibrate | SIDE 카메라 ArUco 마커 기반 3개 쓰레기통 위치 감지 상태 조회·원위치 등록 | 없음 | 200/400 | `RPA_BIN_POSITION_ENABLED=true`일 때 마커 0·1·2 중 하나가 원위치에서 이탈한 상태가 설정 시간 지속되면 COLLECT, 세 통이 모두 복귀해 안정되면 MANAGE로 자동 전환하고 MODE_CHANGED 브로드캐스트. 수동 모드 변경 우선, 카메라 장애 시 현재 모드 유지. 신규 API이므로 배포 전 CTO 검토 필요 |

### EP-02. POST /api/events — 이벤트 생성

Request(EventCreate): cameraId(CameraId), eventCategory(EventCategory), detectionId(str, **구현 완료** — 탐지 파이프라인이 부여하는 중복 저장 방지 키, DB 유니크), trackingId(int|null, misclassification만 — YOLO26 추적 ID), detectedClass(DetectedClass, misclassification일 때만 필수), binId(str, misclassification·overflow 공통 — 현재 물리 통 3개 중 하나), binType(BinType), isMisclassified(bool, misclassification일 때만), confidenceScore(float 0~1, misclassification일 때만), overflowDuration/overflowThreshold(float|null, overflow만), modelVersion(str), imageFileId(str|null, 선택)
