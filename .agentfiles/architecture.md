...
| `CameraId` | `ELEV-TOP` / `ELEV-SIDE` 2개 |
| 카메라 구성 | 카메라 1대 = 지점 1개 = `CameraId` 1개 = 독립 라즈베리파이 1대. `.env` 키는 하이픈 제거(`CAMERA_SOURCE_ELEVTOP`) |
| 메인보드 | 라즈베리파이(Jetson Orin 발주 취소). 추론 없음 — 캡처+RTSP 송신+GPIO/스피커만 |
...
| 주체 | 하는 일 |
|---|---|
| 라즈베리파이 | 캡처 + RTSP 송신 + GPIO(전구)/스피커. **추론 없음**, RTSP는 로컬 백엔드로만(GPU와 직접 연결 안 함) |
...
추론 없음 — 캡처+RTSP 송신+GPIO/스피커만. RTSP는 **TOP/SIDE 둘 다 로컬 백엔드로만** 보냄.
systemd 자동 기동까지 검증 완료, GPIO 전구 연동은 미착수.
