...
1. **자동 라벨링 검증(진행 중, 역할 축소됨)**: 이미지 폴더 → 전처리+자동 라벨링 도구로 1차
   ...
   프롬프트만 사용**(파인튜닝 없음).
   - **LLM이 하는 일은 프레임 단위 "분류 검증"뿐 — 좌표(bbox)는 요구하지도, 쓰지도 않는다**
     (2026-08-28 확정). `decision`/`predictedClass`/`issues`/`confidence` 네 값만 받는다.
     정밀 로컬라이제이션은 VLM의 구조적 약점이라 실측에서 위치 정확도가 사용 불가 수준이었고
     (IoU 중앙값 0.00), 박스를 내놓으라고 요구하면 **없는 물체를 confidence 0.95로 만들어내는
     환각**까지 나왔다. 박스 작성은 사람 검수 UI의 드래그 기능이 전담한다
   - **`confidence`는 신뢰 신호로 쓸 수 없다** — 환각에도 0.95가 붙었다. `minimumReviewConfidence`
     임계값은 보조 장치일 뿐 보호 수단이 아니며, LLM 판정은 어디까지나 참고용이고 최종 결정은
     항상 사람 검수가 내린다
