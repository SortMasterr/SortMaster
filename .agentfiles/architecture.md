...
- 현재 설치 수량인 쓰레기통 3개에는 SIDE 카메라를 향한 면에 ArUco 마커 0·1·2를 부착한다.
  백엔드의 `binPositionMonitorService`가 최초 등록된 원위치와 현재 좌표를 비교해, 한 통이라도
  일정 시간 이동·분리되면 `COLLECT`, 세 통이 모두 원위치에서 안정되면 `MANAGE`로 자동 전환한다.
  사람이 모드 버튼을 누른 경우 그 수동 선택을 우선하며, 카메라 장애 시에는 모드를 변경하지 않는다.
  논리적 `BinType` enum 4종은 기존 이벤트 호환성을 위해 유지하고, 실제 3개 통과 종류의 배치 매핑은
  설치 설정으로 관리한다(사용자가 설치 수량을 3개로 정정, 구체적인 종류 통합 방식은 별도 확정 필요).

## 이벤트 적재

- 매 프레임 Insert 금지, 판정 시점만 저장
- `eventCategory`로 구분: misclassification(투기, 분류 결과 포함) / overflow(넘침, 분류 없이 영상만)
- **현재 설치된 물리 쓰레기통 3개**(`binId`)가 옆 카메라(`ELEV-SIDE`) 시야
  안에 고정 설치. 논리적 `BinType`은 기존 데이터 호환을 위해 4종을 유지하며 실제 3개 통의 종류
  배치 매핑은 설치 설정으로 관리한다. "플라스틱·캔" 통(`binType=recyclables`)은 캔과 플라스틱을 물리적으로 같이
