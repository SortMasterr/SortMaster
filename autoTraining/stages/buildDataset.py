"""YOLO 데이터셋 빌드 단계를 외부에서 호출하기 위한 얇은 어댑터입니다.
...
class PipelineContext(Protocol):
    ...
    def build(self) -> None:
        """TrainingPipeline.build 메서드와 동일한 계약입니다."""
        ...


def buildDataset(pipeline: PipelineContext) -> None:
    """기존 데이터와 승인된 신규 데이터를 병합하고 train/val/test 데이터셋을 만듭니다.
