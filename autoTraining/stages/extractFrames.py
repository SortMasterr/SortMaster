...
class ExtractFramesStage:
    def extract(self)->None:
        ...
        saveEvery=max(1,int(frameConfig["save_every_n"]))
        jpegQuality=int(frameConfig["jpeg_quality"])
        videoPaths=sorted(path for path in self.videos_dir.rglob("*") if path.is_file() and path.suffix.lower() in videoExtensions)
        if not videoPaths:
            raise FileNotFoundError(f"영상이 없습니다: {self.videos_dir}")
        totalCount=0
        # 추출 결과를 list에 누적하지 않고 프레임 저장 직후 JSONL 한 행을 기록한다.
        with ManifestWriter(self.frames_manifest) as writer:
            for videoPath in videoPaths:
                videoKey=videoPath.stem
                outputDirectory=self.frames_root/videoKey
                ...
                try:
                    while True:
                        ...
                        writer.write({"id":itemId,"video":videoKey,"video_path":str(videoPath.resolve()),"frame_index":sourceIndex,"timestamp_seconds":sourceIndex/sourceFps if sourceFps>0 else None,"fps":sourceFps,"image_path":str(imagePath.resolve())})
                        savedCount+=1
                        totalCount+=1
                        sourceIndex+=1
                finally:
        ...
        print(f"[EXTRACT] 전체 {totalCount}개 프레임 보존: {self.frames_root}")
