"""기존 YOLO 자동 라벨링 단계를 외부에서 호출하기 위한 얇은 어댑터입니다.
...
class PipelineContext(Protocol):
    ...
    def label(self) -> None:
        """TrainingPipeline.label 메서드와 동일한 계약입니다."""
        ...


def autoLabel(pipeline: PipelineContext) -> None:
    """현재 기준 YOLO 모델로 후보 이미지의 초안 bbox와 클래스 라벨을 생성합니다.
