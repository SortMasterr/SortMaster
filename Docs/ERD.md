...
> **MVP 기준 GPU 서버 `inference`의 YOLO26**이 투척 추적+쓰레기 종류 분류를 계속 수행하고,
> 로컬 백엔드가 통 상태/쿨다운과 종합해 최종 판정(메인보드가 라즈베리파이로 바뀌면서
> 엣지가 아니라 GPU 서버가 추론 주체, LLM은 미사용). Qwen3-VL-8B(LLM)는 MVP엔 없고 고도화 단계
...
erDiagram
    ...
    BIN_STATES {
        ...
        string sessionId "넘침 감지 모델(GPU 서버 `inference`) 프로세스 시작 시 새 UUID 생성 — 재시작/재연결마다 갱신(확정)"
    ...
    EVENT {
        ...
        string detectionId UK "신규 — GPU 서버 `inference`가 감지 시점에 UUID로 생성(eventId와 동일 방식, 확정). trackingId 조합 방식은 세션 종속이라 배제. 유니크 인덱스로 중복 저장 방지 — GPU 서버↔중앙 백엔드 신호 전달 방식이 아직 TBD라 at-least-once 가능성 대비"
        ...
        string detectedClass "nullable, misclassification만. MVP는 YOLO26(GPU 서버 `inference`)이 직접 분류(쓰레기 아니면 해당 표기), 고도화 단계엔 Qwen3-VL-8B 학습 보조로 정확도 개선 예정. general/paper/plastic/can(신규 — 캔은 플라스틱과 별도 클래스로 이미 학습 중, 아래 참고)/coffeeCup — 총 5종. mixed/uncertain은 제외 확정(아래 참고)"
        ...
        float confidenceScore "nullable, misclassification만, 0.0~1.0. MVP는 YOLO26(GPU 서버 `inference`) 분류 신뢰도"
...
- 탐지 모델(YOLO26=GPU 서버 `inference` 상시 추론, Qwen3-VL-8B=고도화 단계 GPU 서버 학습 보조) 자체는 DB에 영속화되는 대상이 아니라 ERD 범위 밖.
...
- **`detectionId`(중복 방지)/`trackingId`(YOLO 추적 ID)/`modelVersion` 필드 추가 확정** →
  `detectionId`는 DB 유니크 인덱스로 중복 저장 방지(GPU 서버↔중앙 백엔드 신호 전달이
  at-least-once일 가능성 대비, 시간 기반 Cooldown과는 별개 목적). `trackingId`는 디버깅/추적용(전역
...
- **`BIN_STATES`는 `binId`당 최신 상태 1행만 유지(upsert) 확정** → 상태 변경 이력은
  `EVENT`(overflow 카테고리)가 이미 담당해서 중복 보관 불필요. `sessionId`는 GPU 서버
  `inference` 프로세스 시작 시 새 UUID 생성으로 확정
...
- **`detectionId` 생성 방식 확정** → GPU 서버 `inference`가 감지 시점에 UUID 생성(`eventId`와 동일 방식).
