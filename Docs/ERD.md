...
> **NORMAL→FULL로 전환되는 순간에만** 넘침 이벤트 생성+알림. 위 카메라(`ELEV-TOP`)는
> **MVP 기준 엣지(젯슨) YOLO26 단독**으로 투척 추적+쓰레기 종류 분류+판정을 전부 완결(백엔드가
> 아니라 엣지가 판정 주체, GPU/LLM 호출 없음). Qwen3-VL-8B(LLM)는 MVP엔 없고 고도화 단계
> 학습 보조용으로만 후순위 계획(`architecture.md`의 "LLM 활용" 참고). 이벤트는 여전히
...
erDiagram
    ...
    BIN_STATES {
        ...
        string sessionId "넘침 감지 모델(엣지) 프로세스 시작 시 새 UUID 생성 — 재시작/재연결마다 갱신(확정)"
    ...
    EVENT {
        ...
        string detectionId UK "신규 — 엣지가 감지 시점에 UUID로 생성(eventId와 동일 방식, 확정). trackingId 조합 방식은 세션 종속이라 배제. 유니크 인덱스로 중복 저장 방지 — 젯슨↔중앙 신호 전달 방식이 아직 TBD라 at-least-once 가능성 대비"
        ...
        string detectedClass "nullable, misclassification만. MVP는 YOLO26(엣지)이 직접 분류(쓰레기 아니면 해당 표기), 고도화 단계엔 Qwen3-VL-8B 학습 보조로 정확도 개선 예정. general/paper/plastic/can(신규 — 캔은 플라스틱과 별도 클래스로 이미 학습 중, 아래 참고)/coffeeCup — 총 5종. mixed/uncertain은 제외 확정(아래 참고)"
        string binId FK "물리 통 4개 중 하나 — misclassification: 엣지가 추적한 실제 투척 통 / overflow: 어느 통이 가득 찼는지. 과거 초안의 thrownBinId를 대체, 두 카테고리 모두 사용"
        string binType "general/plasticCan/coffeeCup/paper — BIN_STATES.binType과 동일 값 비정규화 저장"
        boolean isMisclassified "nullable, misclassification만. 엣지가 detectedClass→binType 매핑(plastic/can 둘 다 plasticCan에 매핑 등, 아래 참고)으로 변환 후 binId의 실제 binType과 비교해 판정한 결과를 그대로 저장"
        float confidenceScore "nullable, misclassification만, 0.0~1.0. MVP는 YOLO26(엣지) 분류 신뢰도"
...
- **`detectionId`(중복 방지)/`trackingId`(YOLO 추적 ID)/`modelVersion` 필드 추가 확정** →
  `detectionId`는 DB 유니크 인덱스로 중복 저장 방지(젯슨↔중앙 신호 전달이 at-least-once일
  가능성 대비, 시간 기반 Cooldown과는 별개 목적). `trackingId`는 디버깅/추적용(전역
...
- **`BIN_STATES`는 `binId`당 최신 상태 1행만 유지(upsert) 확정** → 상태 변경 이력은
  `EVENT`(overflow 카테고리)가 이미 담당해서 중복 보관 불필요. `sessionId`는 엣지 프로세스
  시작 시 새 UUID 생성으로 확정
...
- **`detectionId` 생성 방식 확정** → 엣지가 감지 시점에 UUID 생성(`eventId`와 동일 방식).
