...
| 메인보드 | **Jetson Orin Nano Super 발주 건 완전 취소 → 라즈베리파이로 확정 대체.** YOLO26 추론을 GPU 서버로 이관하면서(아래 "탐지 파이프라인" 참고) 메인보드엔 고성능 추론이 더 이상 필요 없어짐 — 캡처+RTSP 송신+스피커만 담당(전구/LED는 GPIO 제약상 방향에서 제외). Raspberry Pi OS(Python 3.11+)라 `WebApps/backend`와 문법 호환성 문제 없음 |
...
> 추론 성능이 부족해 **캡처+RTSP 송신+스피커만 담당**(GPIO 전구/LED는 방향에서 제외)하고, **YOLO26 상시 추론
...
- **역할 분담**:
  - **라즈베리파이(엣지, TOP+SIDE 공통)**: 캡처+RTSP 송신+스피커(경고음) — GPIO 전구/LED
    연동은 방향에서 제외(라즈베리파이는 USB 외 GPIO로 LED 구동 불가) —
...
> NPU/GPU가 더 이상 필요 없어짐 — 캡처+RTSP 송신+스피커만 하면 되는 역할이라 라즈베리
> 파이로 충분.
...
3. 로컬 백엔드로부터 RPA 트리거 신호 수신 → **스피커**(USB 또는 3.5mm 오디오잭, Python에서
   `aplay` 서브프로세스 등으로 경고음 재생) 출력. 스피커 검증용 리스너는
   `debug/hardware/alertListener.py`에 구현되어 기존 `/ws/events`의
   `MISCLASSIFICATION_DETECTED` 수신 시 `aplay`로 경고음을 재생한다(실이벤트 연동 확인됨).
   실제 설치 환경 상시 서비스화는 아직 미착수. **GPIO로 전구(LED)를 켜는 구성은 방향에서
   완전히 제외** — 애초 Jetson Nano 기준으로 세운 계획이었으나, 라즈베리파이는 GPIO로
   LED를 직접 구동할 수 없어(USB 외 연결 불가) 폐기(2026-08-30, `decisionLog.md` 참고).
...
**구현 상태: 스피커는 프로토타입 존재·상시 서비스화 미착수. 전구(LED)는 방향에서 완전히
제외.** 위 "오분류 시 경고음 즉시 자동 트리거"가 목표 설계이고, 실제로 모드 전환에 맞춰
스피커를 자동으로 트리거하는 코드(`services/rpaService.py`)는 아직 없다(README "현재 상태"
표의 "RPA(스피커 경고음)"/"RPA(전구/LED)" 항목 참고). `POST /api/mode`로 모드 전환 자체는
되지만, 그 전환이 실제 스피커 출력으로 자동 이어지는 경로는 아직 구현되지 않았다 — 스피커
프로토타입 코드는 위 "메인보드(라즈베리파이) 엣지 코드"의 3번 항목 참고.
...
`RPA 트리거` 단계는 위 "RPA 정책"에서 설명한 목표 설계를 가리키는 것으로, 실제 스피커
자동 연동 코드가 붙기 전까지는 이 화살표가 현재 아무 동작도 하지 않는다(WS Broadcast만
실제로 일어남).
...
- 스피커 상시 서비스화 방식, 라즈베리파이↔중앙 백엔드(RPA 트리거) 신호 전달 방식
  (MQTT/HTTP/WS 중 미정, GPU 서버↔백엔드는 HTTP POST로 확정됨 — 위 "탐지 파이프라인" 참고).
  전구(LED) HW/GPIO 연동은 2026-08-30 방향에서 완전히 제외로 확정(더 이상 TBD 아님,
  `decisionLog.md` 참고)
- 안면인식 레포 포함 여부
...
- **GPU 판정 지연이 현장 알림 지연으로 이어지는 문제** — RPA 알림은 `EVENT` 생성 시점에
  트리거되는데 그 생성이 GPU의 `aiDisposal` 도착을 기다리므로, GPU가 느리면 경고음도
