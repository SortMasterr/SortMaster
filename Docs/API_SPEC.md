...
* **상시 감시 모델**
  ...
  * YOLO26 사용 예정(변경 전 YOLOv8-Nano)
  * 손 감지 조건 폐지 — 쓰레기 감지 자체가 트리거
  * 옆 카메라가 넘침 상태 감지 → 위 카메라가 어느 통인지 위치 특정 → 넘침 이벤트
  * YOLO26이 쓰레기 감지 → 비동기 LLM 분류 + 투척 위치 추적 → 위치와 분류 결과 불일치 시 투기 이벤트(상세는 `.agentfiles/architecture.md`의 "탐지 파이프라인" 참고)
